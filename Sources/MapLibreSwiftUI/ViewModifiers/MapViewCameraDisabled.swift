...
public extension View {
    /// Prevent Mapvina-DSL from updating the camera, useful when the underlying ViewController is managing the camera,
    /// for example during navigation when Mapvina-Navigation is used.
    ///
    /// - Parameter disabled: if true, prevents Mapvina-DSL from updating the camera
    @MainActor
