...
## MapVinaDeveloper SwiftPM trait toggle

When testing this package, you'll want to change `let enableDeveloperTools = false` 
to `true`. This variable enables the `"MapVinaDeveloper"` SwiftPM trait, allowing you to 
...
This package is structured into a few targets. `InternalUtils` is pretty much what it says. `MapVinaSwiftDSL` and
`MapVinaSwiftUI` are published products, and make up the bulk of the project. Finally, `Examples` is a collection of
SwiftUI previews. 

The DSL provides a more Swift-y layer on top of the lower level MapVina APIs, and features a number of
...
these [ViewModifiers](Sources/MapVinaSwiftUI/ViewModifiers). This enables developers to modify a
...
make sure that the `MapVinaSwiftUI-Package` target is active.
...
These are a bit tricky due to the async nature of MapVina and integrating this into an Xcode UI test is challenging.
